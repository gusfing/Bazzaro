...
export async function getProducts(): Promise<Product[]> {
  ...
  try {
    ...
    // For debugging, let's temporarily force the fallback
    console.log('getProducts: Using MOCK_PRODUCTS fallback for debugging');
    console.log('getProducts: MOCK_PRODUCTS loaded, length:', MOCK_PRODUCTS.length);
    console.error('URGENT DEBUG: MOCK_PRODUCTS length:', MOCK_PRODUCTS.length);
    return MOCK_PRODUCTS;

    /* Temporarily disabled Firestore call for debugging
    const products = await ProductsService.getAll();
    // If Firestore returns empty array, fall back to mock data for development
    if (!products || products.length === 0) {
      console.log('Firestore empty, using MOCK_PRODUCTS as fallback');
      return MOCK_PRODUCTS;
    }
    return products;
    */
