...
import { ChatSession } from '../lib/openrouter';
...
const SupportChatWidget: React.FC = () => {
    ...
    const initializeChat = async () => {
      try {
        const newChat = new ChatSession(
          'You are a friendly and helpful BAZZARO support assistant specializing in luxury fashion. You are knowledgeable about materials, craftsmanship, and order details. Keep responses concise and helpful.'
        );
        setChat(newChat);
        setMessages([
          {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
        const stream = chat.sendMessageStream(inputValue);
        let modelResponse = '';
