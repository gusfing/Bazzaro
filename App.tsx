...
import { initializeStore, saveAbandonedCart } from './lib/store';
...
const AppContent: React.FC = () => {
  ...
  // Effects to save state to localStorage on change (debounced)
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      try { 
        localStorage.setItem('bazzaro_cart', JSON.stringify(cartItems)); 
      } catch (e) { 
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      try { 
        localStorage.setItem('bazzaro_wishlist', JSON.stringify(wishlist)); 
      } catch (e) { 
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      try { 
        localStorage.setItem('bazzaro_wallet', JSON.stringify(walletBalance)); 
      } catch (e) { 
        console.warn('Failed to save wallet to storage', e) 
      }
    }, 300);
    return () => clearTimeout(timeoutId);
  }, [walletBalance]);

  // Track cart abandonment - save cart as abandoned after 5 minutes of inactivity
  useEffect(() => {
    if (cartItems.length === 0) return;

    const timeoutId = setTimeout(() => {
      // Only create abandoned cart if user is not authenticated (to avoid spam)
      if (!currentUser && cartItems.length > 0) {
        ...
        saveAbandonedCart(abandonedCart);
      }
    }, 5 * 60 * 1000); // 5 minutes

    // Cleanup timeout on unmount or cart change
    return () => clearTimeout(timeoutId);
  }, [cartItems.length, currentUser]); // Only depend on cart length, not full cart array
        ...
        <main className="flex-grow flex flex-col">
          <AnimatePresence mode="wait">
            <Routes location={location} key={location.pathname}>
              ...
              <Route path="/admin/cart-abandonment" element={<AdminLayout><AdminCartAbandonment /></AdminLayout>} />
